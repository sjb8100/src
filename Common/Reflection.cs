    ...
    public static class Reflection
    {
        ...
        public static bool IsVisible(this Type type)
        {
            ...
            return type.IsVisible;
        		...
        		/// <summary>
		/// 确定当前的类型的实例是否可以从指定类型的实例赋值
		/// 返回true的情况：
		///		给定类型和当前类型相同
		///		当前类型是给定类型的父类
		///		当前类型是接口，并且给定类型实现了该接口
		///		给定类型符合当前类型的泛型参数约束
		/// </summary>
		/// <param name="parent"></param>
		/// <param name="sub"></param>
		/// <returns></returns>
		public static bool IsAssignableFrom(this Type parent, Type sub)
		{
#if (UNITY_METRO && !UNITY_EDITOR) || WINDOWS_UWP
			return sub != null && parent.GetTypeInfo().IsAssignableFrom(sub.GetTypeInfo());
#else
