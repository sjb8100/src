...
namespace Common
{
	...
	public class ObservableDictionary<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>
	{
		...
		public event EventHandler<EventArgs<KeyValuePair<TKey, TValue>>> ItemUpdate;
		...
		private void OnItemUpdate(TKey key, TValue value)
		{
			if (ItemUpdate != null)
				ItemUpdate(this, new EventArgs<KeyValuePair<TKey, TValue>>() { Data = new KeyValuePair<TKey, TValue>(key, value) });
		}
