    ...
    internal abstract class AttributeMap
    {
        ...
        private sealed class ReflectionAttributeMap : AttributeMap
        {
            ...
            public override bool TryGet(string key, bool publicOnly, out object value)
            {
                ...
                if (null != attribute)
                    members = Helpers.GetInstanceFieldsAndProperties(attribute.GetType(), publicOnly);
                foreach (MemberInfo member in members)
                {
...
#endif
                    {
                        ...
                        if (prop != null) {
                            MethodInfo propMethodInfo = Helpers.GetGetMethod(prop, false, false);
                            //value = prop.GetValue(attribute, null);
                            if (null == propMethodInfo)
                            {
                                propMethodInfo = Helpers.GetGetMethod(prop, false, true);
                            }
                            if (null != propMethodInfo)
                            {
                                value = propMethodInfo.Invoke(attribute, null);
                                return true;
                            }
                        }
